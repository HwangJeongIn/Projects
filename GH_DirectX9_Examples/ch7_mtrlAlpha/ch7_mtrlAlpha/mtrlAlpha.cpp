...
bool Setup()
{
	...
	// 난반사 컬러로 알파를 계산한다.
	// 재질을 통해서 알파를 계산한다. // 위에서 재질을 정의해주었다. 그재질의 DIFFUSE.a을 기준으로 계산한다.
	Device->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE);

	// 텍스처를 입혀서 해도 되지만, 티포트의 버텍스값을 가지고 있지 않기때문에 매핑시키기가 어렵다
	// 알파값이 적용된 텍스처 붙이면 희미하게 보이는 수준으로 렌더링 된다.
	...
	// 블렌드 인수 설정 > 알파 성분으로 투명도 조절할 수 있게 함
...
bool Display(float timeDelta)
{
	if( Device )
	{
		...
		// Draw the teapot
		// 알파모드는 그릴것만 그려주고 바로 꺼주는게 좋다.
		// 이때 한꺼번에 처리해주는게 좋다. // 여러번 켜고 끄는것은 바람직 하지 않다.
