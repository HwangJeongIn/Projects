...
프로젝션 변환은 버텍스를 뷰포트에 사용되는 좌표계로 변환
 */
d3d::Ray CalcPickingRay(int x, int y)
{
	...
	*/
	...
	// 스크린좌표에서 투영창 좌표로 변환시키기 위해서
	// 스크린에 표시된 좌표가 어떻게 나왔는지 생각해본다
...
LRESULT CALLBACK d3d::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		// ch12의 카메라의 움직임이 있는 예제와 달리 뷰포트행렬을 고정시켜서 한곳만 바라본다.
		// 만약에 카메라가 움직여야 하는 상황이라면 뷰포트 행렬도 카메라에 따라서 계속 갱신해주어야한다.
		// 그렇게 해야 올바른 뷰포트 역행렬을 구할 수 있고, 정확한 레이캐스트가 가능하다.
		...
		// 월드 좌표계 물체를 뷰포트좌표로 변환하고 그를 기준으로 계산해 주었다.
		// 계산이 완료되었기 때문에 다시 레이를 월드 좌표계로 변환하기 위해서
		// 뷰포트 > 월드좌표계로 변경해준다.
		// 그를 위해서 윌드좌표계 > 뷰포트의 행렬의 역행렬(월드 좌표계 > 뷰포트)을 적용시켜서 레이를 변환한다.
