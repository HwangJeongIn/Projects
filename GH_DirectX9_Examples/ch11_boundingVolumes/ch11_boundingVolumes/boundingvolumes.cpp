...
bool ComputeBoundingSphere(ID3DXMesh* mesh, d3d::BoundingSphere* sphere)
{
	...
	hr = D3DXComputeBoundingSphere(
		// 버텍스들의 위치를 나타내는 버텍스 배열의 첫번째 버텍스 벡터를 가리키는 포인터
		(D3DXVECTOR3*)v,
		// 버텍스 배열 내 버텍스의 수
		mesh->GetNumVertices(),
		// 버텍스를 계산하기 전에 부가적인 정보를 건너뛰기 위해서 지정해준다.
		// 여기서는 구조체의 시작에 버텍스 ㅜ이치 속성이 보관되어 있다고 가정하고 있다.
		D3DXGetFVFVertexSize(mesh->GetFVF()),

		// 내부에서 초기화 되게 하기 위해서 주소값을 넘겨준다.
		&sphere->_center,
		&sphere->_radius);
...
bool ComputeBoundingBox(ID3DXMesh* mesh, d3d::BoundingBox* box)
{
	...
	hr = D3DXComputeBoundingBox(
		...
		// 구체로 경계를 구할때와 비슷하나 여기서는 박스의 두 지점을 초기화한다.
		&box->_min,
		&box->_max);
