...
struct MultiTexVertex
{
	...
	// 3개의 텍스처를 붙이길 원한다면 3개의 uv좌표가 필요하다
	// 고정기능 파이프라인은 최대 여덟개 까지의 텍스처 좌표를 지원한다. // D3DFVF_TEX8
	float _u0, _v0;
	float _u1, _v1;
	float _u2, _v2;
...
bool Setup()
{
	...
	// 멀티텍스처를 이용하여 처리할것이기 때문에 조명은 필요없다.
...
bool Display(float timeDelta)
{
	if( Device )
	{
		...
		추측
		내부적으로 
		D3DFVF_XYZ	<=> POSITION0

		D3DFVF_TEX3	<=> TEXCOORD0
					<=> TEXCOORD1
					<=> TEXCOORD2
		...
		*/
		...
		// base tex
		// 세부정보에서 레지스터 인덱스를 사용하여 셈플러 객체를 초기화시킨다.
