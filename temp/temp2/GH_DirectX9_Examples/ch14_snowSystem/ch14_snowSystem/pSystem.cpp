...
void PSystem::render()
{
	...
	if( !_particles.empty() )
	{
		...
		// 현재 잠글 위치를 나태낸다.
		// 만약 현재 잠글위치가 버텍스 버퍼의 크기보다 크거나 같으면 초기위치로 돌린다.
		...
		_vb->Lock(
			...
			(void**)&v,
			// 0일때는 D3DLOCK_DISCARD 
			// 0이 아닐때는 D3DLOCK_NOOVERWRITE < 세그먼트1이 렌더링 되는동안 세그먼트2를 잠그고 채울 수 있다.
		...
		for(i = _particles.begin(); i != _particles.end(); i++)
		{
			...
			if( i->_isAlive )
			{
				...
				if(numParticlesInBatch == _vbBatchSize) 
				{
					...
					// 만약에 옵셋이 최대사이즈를 넘어간다면 옵셋을 0으로 초기화한다
					if(_vbOffset >= _vbSize) 
						_vbOffset = 0;       

					// 다시 기본세팅을 해준다.
					// 현재 위치부터 한세그먼트의 배치 사이즈만큼 락걸고 기본세팅을 해준다.
					_vb->Lock(
						_vbOffset    * sizeof( Particle ),
						_vbBatchSize * sizeof( Particle ),
						(void**)&v,
		...
		{
			// 조건이 만족되지 못해서 마지막 단계가 렌더링되지 않는 경우가 발생할 수 있다.
