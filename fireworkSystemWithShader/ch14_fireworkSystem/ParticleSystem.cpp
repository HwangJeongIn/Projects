...
ParticleSystem::ParticleSystem(IDirect3DDevice9 * device, /*const D3DXMATRIX & viewMatrix,*/ const char * textureFileName, int numOfParticles, int particleSize, const D3DXVECTOR3 & origin)
	: device(device), vb(nullptr), texture(nullptr), /*declaration(nullptr),*/
	maxNumOfParticles(numOfParticles), currentNumOfParticles(0), particleSize(particleSize), origin(origin), amountFactor(0.5f)
{
	// 원래라면 mainCamera에서 device / viewMatrix / projectionMatrix를 받아온다. // 원래의 인풋 :: mainCamera / textureFileName
...
void FireExPlosion::resetParticle(ParticleAttribute & attribute)
{
	...
	attribute.velocity *= d3d::GetRandomFloat(1.5f, 50.0f);

	attribute.color = D3DXCOLOR(
		d3d::GetRandomFloat(0.8f, 1.0f),
		d3d::GetRandomFloat(0.0f, .3f),
		d3d::GetRandomFloat(0.0f, .3f),
		1.0f);

	attribute.colorFade = D3DXCOLOR(
		d3d::GetRandomFloat(0.0f, .30f),
		d3d::GetRandomFloat(0.0f, .30f),
		d3d::GetRandomFloat(0.8f, 1.0f),
		.0f);
	...
	// 수명 2초동안 유지하도록 설정
	attribute.age      = 0.0f;
	attribute.lifeTime = d3d::GetRandomFloat(3.0f, 3.0f);// lives for 2 seconds
}
